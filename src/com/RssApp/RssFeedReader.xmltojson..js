  ...
  useEffect(() => {
    const fetchRSSFeed = async () => {
      try {
        ...
        // Extract items (adjust based on your JSON structure)
        const items = (data.items || []).map((item) => ({
          title: item.title || "No title",
          link: item.link || "#",
          pubDate: item.pubDate || item.pub_date || "", // Handle variations
          description: item.description || item.content || "", // Handle variations
          thumbnail: getThumbnail(item), // Add thumbnail
        }));
        setFeedItems(items);
        setLoading(false);
        setError(null);
